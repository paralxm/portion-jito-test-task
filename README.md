...
- [<img src="docs/assets/readme/icons/figma.svg" width="18" height="18" align="absmiddle" alt=""> **UI Kit**](https://www.figma.com/design/heuO3V1WlKG44CukQswCkw/jito-calories-calculator?node-id=28-2253&t=urjsytdSqhdIRbXu-1) — the editable Figma counterpart of the implemented design system, created for design review, handoff, and future iteration.
- [<img src="docs/assets/readme/icons/figma.svg" width="18" height="18" align="absmiddle" alt=""> **High-fidelity screens**](https://www.figma.com/design/heuO3V1WlKG44CukQswCkw/jito-calories-calculator?node-id=3-2&t=urjsytdSqhdIRbXu-1) — the editable Figma counterpart of the implemented product screens, including the main prototype flows and transitions for review and handoff.
...
The sequence was research first, then implementation directly in code — not a conventional "finish Hi-Fi in Figma, then build" handoff. Secondary research and a six-app competitive teardown (FigJam) fed a synthesis of four design hypotheses and derived UX requirements; those became two task-flow diagrams and a low-fidelity screen/state inventory (Figma); a stylescape compared two visual directions and selected one. From there, the Hi-Fi visual system was built and iterated **in code and Storybook** against that stylescape — tokens, then primitives, components, patterns, templates and screens — with each revision (the initial redesign, a "Stage B" populated Food tab, revisions R1–R6, H2 and T1) closing on a runtime walkthrough script, a Storybook capture script, and a written ledger entry recording what was checked and found.

After the code-first implementation was completed and verified, the final High-Fidelity screens and core UI Kit were recreated in Figma as editable counterparts for design review, cross-functional reference, and future iteration. The transfer was guided by the documented [figma-transfer-contract.md](docs/design/figma-transfer-contract.md). The repository and Storybook remain the primary source of truth for implementation details and complete component-state coverage.

The repository's own last recorded full verification run (typecheck, token/drift check, unit and Storybook Vitest projects with axe at error severity, app and Storybook builds, and the contrast matrix — [design-system guide §6](docs/design-system/README.md)) is dated 2026-09-04. By that point, the code-first implementation and Storybook system were complete. The remaining work through 7 September focused on transferring the finalized UI into Figma, documenting the resulting design assets and prototype flows, and preparing the final submission. Counts quoted above (88 story files, 20 test files, 61 tracked runtime screenshots, 167 tracked Storybook captures) were counted directly from the repository while writing this document.
...
- **Figma Assets & Documentation:** This was a code-first project, so the repository and Storybook remain the primary source of truth for implementation details and complete component-state coverage. The Figma file contains the corresponding editable UI Kit and High-Fidelity screens, including prototype flows and transitions for design review, cross-functional reference, and future iteration.

The most direct way to understand how these decisions work together is to open the interactive prototype, run both core user journeys, and compare the Storybook component states with how those components behave in the product flows.
