...
- Targets come from the targets flow: the route-choice sheet, then focused full-screen steps — the manual editor with *suggested* preset grams, or the three-step estimate (compact step bar, selection cards, unit selectors in the label rows) with its full-screen review (small "Estimated" label, the centred figure, Adjust in place); Help carries the method, the main path never shows formula names; nothing is called personalised or medical.
