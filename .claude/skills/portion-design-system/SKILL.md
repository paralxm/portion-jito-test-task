...
- Collections of items get one presentation switch (`ViewToggle`, a radio group) and share data, order and open behaviour between `FoodResultRow` (list) and `FoodCard` (grid), and between `RecipeCard` `row` (list) and `tile` (grid and rails); filters live in the shared sheet pattern (`RecipeFiltersSheet`, `FoodFiltersSheet`) opened from the results toolbar, with a draft, Apply, Clear/Reset and dismissal that keeps the applied state; multi-select quick chips are `FilterChip` toggles (`aria-pressed`) over the same criteria model, an exclusive set (time) is the same chip toggled one at a time.
- `NutritionMacros` has `row` and `cards` presentations; `DailyNutrition` composes the calorie card and the macro cards; the targets flow is `TargetsEntrySheet` (the route choice) → `EstimateStepScreen` (About you / Your activity / Your goal on `FocusedFlowLayout` with `FocusedBar`, `SelectionCard`, `UnitField`, `HelpDialog`) → `TargetEditorScreen` (review / manual / edit over `MacroTargetsEditor` and `EffectiveDateControl`); the energy estimate and the macro presets are pure domain modules (`energy-estimate.ts`, `macro-presets.ts`) with their sources in `docs/ux/targets-and-estimation.md`.
- The portion step is one component (`PortionForm` over `usePortionDraft`): documented − / + increments, presets only from the item's own units, the live `NutritionSummary`, `MealPicker` and the target-day line. Every commit surface reuses it; the final action stays in the screen footer. `MethodOption` has `row` and `card` presentations and a `quiet` tone; `DayStrip` is one scrolling radio group of 48 px day tiles with no week buttons; `DiscardChangesDialog` is the one exit confirmation over `ConfirmDialog`.
