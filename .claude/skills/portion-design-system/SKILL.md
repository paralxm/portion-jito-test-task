...
- Collections of items get one presentation switch (`ViewToggle`, a radio group) and share data, order and open behaviour between `FoodResultRow` (list) and `FoodCard` (grid); `RecipeCard` has `row` and `tile` presentations for lists and rails; filters live in the shared sheet pattern (`RecipeFiltersSheet`, `FoodFiltersSheet`) with a draft, Apply, Clear/Reset and dismissal that keeps the applied state; multi-select quick chips are `FilterChip` toggles (`aria-pressed`) over the same criteria model.
- The portion step is one component (`PortionForm` over `usePortionDraft`): documented − / + increments, presets only from the item's own units, the live `NutritionSummary`, `MealPicker` and the target-day line. Every commit surface reuses it; the final action stays in the screen footer. `MethodOption` has `row` and `card` presentations and a `quiet` tone; `DayStrip` is a radio group of 48 px day tiles; `DiscardChangesDialog` is the one exit confirmation over `ConfirmDialog`.
