...
- `NutritionMacros` has `row` and `cards` presentations; `DailyNutrition` composes the calorie card and the macro cards; `TargetsSheet` owns the targets routes over `MacroTargetsEditor`; the energy estimate and the macro presets are pure domain modules (`energy-estimate.ts`, `macro-presets.ts`) with their sources in `docs/ux/targets-and-estimation.md`.
