...
Updated: 2026-09-05 (R1–R6 revision, §12; Stage B close-out, §11.7; Stage A close-out, §10.9). Baseline pass: 2026-09-04. Branch: `feat/redesign-r1-r6` (from `main` at `30d54d9`, which integrates `feat/hifi-screens`). Owner of this file: the Hi-Fi product pass. It is the compact, resumable record of the state inventory (41 rows in the baseline pass, 46 after the §10 redesign, 51 after Stage B §11, 55 after the R1–R6 revision §12), the transition model, the reference synthesis, the content model, the asset register and every material interpretation. Product truth stays in `PRODUCT.md`; behaviour stays in `docs/ux/`; visual composition stays in `DESIGN.md`. This ledger records how those were applied, not a second specification.
...
| salmon-rice-bowl | `src/assets/images/recipes/salmon-rice-bowl.webp` | `recipe-salmon-rice-bowl` (“Salmon rice bowl”; salmon over rice with coriander; the photo shows the dish, not the exact portion) | Pexels | Gu Ko (`/@gu-ko-2150570603`) | https://www.pexels.com/photo/salmon-and-rice-bowl-with-fresh-cilantro-37332351/ | https://www.pexels.com/license/ | 2026-09-05 | Pexels CDN centre crop 1200 × 900, WebP q0.72 (`.verification/to-webp.mjs`); 16:9 hero by `object-fit: cover` | empty alt beside the visible title |
| yoghurt-parfait | `src/assets/images/recipes/yoghurt-parfait.webp` | `recipe-yoghurt-parfait` (“Greek yoghurt parfait with blueberries and granola”) | Pexels | Jared Kaeb (`/@jared-kaeb-2910117`) | https://www.pexels.com/photo/white-drink-with-blue-berries-9024839/ | https://www.pexels.com/license/ | 2026-09-05 | as above | empty alt |
| overnight-oats | `src/assets/images/recipes/overnight-oats.webp` | `recipe-overnight-oats` (“Overnight oats with berries”; oats with berries and milk in a bowl) | Pexels | Livilla Latini (`/@livilla-latini-1678510737`) | https://www.pexels.com/photo/a-bowl-of-oatmeal-with-berries-and-milk-27850091/ | https://www.pexels.com/license/ | 2026-09-05 | as above | empty alt |
| chickpea-curry | `src/assets/images/recipes/chickpea-curry.webp` | `recipe-chickpea-curry` (“Chickpea and spinach curry”; the photo is a chana masala close-up — spinach is not prominent, so the image is appearance only) | Pexels | Muhammad Khawar Nazir (`/@drmkhawarnazir`) | https://www.pexels.com/photo/close-up-of-chana-masala-9287035/ | https://www.pexels.com/license/ | 2026-09-05 | as above | empty alt |
| vegetable-omelette | `src/assets/images/recipes/vegetable-omelette.webp` | `recipe-vegetable-omelette` (“Vegetable omelette with side salad”) | Pexels | Serdar Göksu (`/@serdargoksu`) | https://www.pexels.com/photo/omelette-with-vegetable-salad-26161037/ | https://www.pexels.com/license/ | 2026-09-05 | as above | empty alt |
...
- Problem: technical slop (arbitrary values, fake chrome, decorative effects) creeping in with six new families → tool: `node .claude/skills/impeccable/scripts/detect.mjs --json src` after Stage A/B/C and again after D/E/F → expected: an empty findings list. Result: `[]` both times (exit 0).
- Problem: composition drift from R1–R6 → tool: the `critique` reference's Assessment A checklist (hierarchy, cognitive load, heuristics) applied inline to the rendered captures of Home, Recipes, the method sheet, manual steps 1–2 and the two reviews during the batched visual review (§12.7) → expected: material fixes only. Result: a degraded single-context pass by the reference's own definition (no sub-agents in this session); findings, all fixed at the owning layer: day-strip tiles squeezed at 200 % (3 rem minimum), compact macros breaking a value from its unit at 320 (17 rem stack), the group action wrapping inside its label, the Food placeholder clipped beside the sibling scanner, the step-2 basis phrase breaking, the steppers misaligned with the input at 200 % (D-57). No heuristic scoring was persisted; nothing was rebranded.
- `adapt` was not invoked: the reflow defects above were fixed at their owning CSS rather than through a separate adaptation pass.

### 12.4 Checkpoint (close-out)

- Requirements: A1–A7, B1–B5, C1, D1–D4, E1–E3, F1, G1 all implemented and verified (§12.7). Nothing incomplete; nothing blocked.
- Branch `feat/redesign-r1-r6` from `main` at `30d54d9`. Commits: `1883bb9` (Stages A, B, C), `fba5861` (Stages D, E, F with scripts and stories), `a90ff33` (verification evidence), then the documentation commit that records this checkpoint. Pushed to `origin/feat/redesign-r1-r6` after the documentation commit; the push result and tip comparison are in the final report.
- Files: see the commits above; the ledger sections 12.1–12.7; `verification/manifest.md`.
- Screenshots: `verification/runtime/` (47 tracked of 64), `verification/storybook/states/` (78: 55 states + 23 variants), `verification/storybook/` (59 component captures).
- Running tasks: none. Blockers: none.
- Next action if resumed: none required for this revision; a later pass would start from `main` after this branch is reviewed and merged by the maintainer (no merge is performed here).

### 12.5 Decisions (continuing §11.5)

- **D-42 Selected day.** Home shows one selected local day up to today (`selectedDay`, `null` = follow today across midnight; an explicitly selected earlier day stays). The week strip is a radio group of the selected day's week; future days are disabled and named as such; previous/next week and `Today` are buttons, so swiping is never the only path. Entries, totals and water come from that day; the meals heading, the water sheet and the toast name the day when it is not today.
- **D-43 Target day binding.** A task binds the day Home showed when it started (`taskOrigin.dayKey`); `createEntry` takes that key, so a midnight rollover during the task never moves the entry; the commit returns Home showing that day.
- **D-44 Goal history.** Goals are effective-dated periods (`goal-history.ts`); a goal edit applies from today onward and keeps earlier periods; a cleared goal is a period too; a day before the first period has no goal and shows logged amounts alone with an explanation. A version-1 record's single goal migrates as a period from the migration day. The sole Set goal / Edit goal action is on the calorie surface (the header's duplicate is removed).
- **D-45 Streak.** `computeStreak`: the consecutive-day run of days with ≥ 1 confirmed food or recipe entry, ending today when today qualifies, otherwise ending yesterday, otherwise 0; recomputed from the entries on every change. Shown as a neutral `CalendarCheck` glyph and count in the header with a rule sheet; never a reward.
- **D-46 Discovery.** The Recipes root derives its groups from each record (Featured is an editorial flag on the record; under 30 minutes and 30 g protein from known values; unknown values never qualify), respects the active criteria, omits empty groups and hands Search a criteria snapshot from every View all. The complete catalogue with its unique count lives in Search's Recipes scope while the query is empty. Counts are computed from unique ids, never hard-coded.
- **D-47 Dietary set.** `RecipeCriteria.dietary` is a canonical set; each constraint is one criterion (`dietary:<id>` keys) and combines with AND; vegan satisfies vegetarian; unspecified data satisfies nothing; `All` clears the set. The quick chips are toggles over the same model as the sheet; the root hides dietary from its applied-chip row because the quick chips already show it.
- **D-48 Sibling scanner.** In the Food scope the scanner is a secondary button with icon and label beside the field (below it under 22 rem), not an in-field action; the field placeholder shortens to "Food or dish" so it is never clipped beside the button at 393 px.
- **D-49 Hero containment.** The details hero cancels only the Container's page inset (never the viewport) and has no negative top margin; RootScreenLayout gives the content no top padding, so the hero starts exactly at the header's bottom edge (57 px at 393 and on desktop; measured).
- **D-50 Two-step manual task.** The task's draft (details, photo, retained portion and unit, meal, provenance) is owned by the app for the task's life; step 1 is controlled by it; step 2 starts from the carried portion (`carryPortion` keeps it when its unit is still supported, otherwise resets to the reference with a note). Continue → Back → Continue keeps the actual portion (the audited reset). Manual candidate ids are reload-unique.
- **D-51 Shared exit policy.** `DiscardChangesDialog` over `ConfirmDialog` with fixed copy; Keep editing focused; Escape/backdrop keep the draft. Step 1 asks when dirty relative to its starting draft or when a photo is attached; step 2 always asks (the task holds data); a new-candidate review asks when the portion, unit or meal changed; existing-entry review keeps its own "Discard the changed amount?" wording. Back within a task (step 2 → step 1, correction draft → review) preserves the draft without asking; Cancel leaves the task to the recorded origin. `Done` is removed.
- **D-52 Browser history.** `useHistoryStack` mirrors the focused-step stack into history entries; popstate consults the visible step's registered guard (`exit-guard.tsx`) and restores the entry when the guard intercepts; a dirty guard also arms `beforeunload`. Limitation: refresh and tab closure get only the browser's generic prompt; native gestures are whatever the browser maps to history.
- **D-53 Portion form.** `usePortionDraft` + `PortionForm` are the one portion step: steps of 25 g/ml, ¼ serving or 1 piece on a snapped grid; presets only from the item's own units (mass steps 50/100/150/200 plus its serving; serving steps ½/1/1½/2 plus 100 of its mass unit); precision internal, rounding in presentation; the calorie value never breaks. Foods commit on their screen; the Add-to-meal sheet stays for recipes only (D-23 narrowed).
- **D-54 Source-specific review.** One hierarchy (R5) with a source badge, record fields only (`brand`, `barcode` when supplied), the captured sample frame for photo results labelled as such, and per-source corrections: barcode → Change product / Edit label values; photo → Change match / Retake photo / Edit nutrition values; search → Change food. Corrections open manual step 1 prefilled (`draftFromCandidate`) with provenance (`kind: 'override'`), producing a manual entry; the catalogue is never mutated.
- **D-55 User photos.** A photo is validated (JPEG/PNG/WebP/HEIC, ≤ 8 MB), previewed through an object URL released when replaced or the task is discarded, and on commit downscaled to a ≤ 240 px JPEG preview in `portion.photos` (at most 40, oldest dropped) — a separate key from the record, resolved by candidate id on load after the catalogue. Nothing is inferred from it.
- **D-56 Method sheet.** `MethodOption` presentations row / card and tone quiet compose the R6 hierarchy; the card pair stacks under 17 rem of body width, keeping the order. The caption is "With the camera" (no "instant recognition" claim), and there is no PRIMARY badge.
- **D-57 Row-layout follow-ups.** Compact macros stack under 17 rem (a "27.6 / 220 g" value never breaks before its unit); the day-strip tiles are at least 3 rem so 200 % text never squeezes them; the discovery group action never wraps inside its label; the step-2 basis phrase is unbreakable; the portion steppers align to the input by rem and move beneath the field under 17 rem.

### 12.6 Inventory — recomputed: exactly 55 rows

51 rows of §11.4 − O05 (the food Add-to-meal sheet, retired: foods commit on review) − S07-6 (review from manual entry, superseded by the manual second step) + S01-5, S01-6, S02-11, S06-4, S06-5, O08 = **55**. Reopened with changed composition: S01-1/S01-2 (strip, streak, order), S03-1/S03-2 (discovery), O01 (R6), S02-1…S02-4 (sibling scanner), S06-1…S06-3 (step 1 of two), S07-1/S07-2/S07-4/S07-5 (R5 + direct commit), S08-1 (hero), O03 (shared copy).

| # | Portion ID | Surface | State / purpose | Entry | Controls → outcome | Mode |
| --- | --- | --- | --- | --- | --- | --- |
| 52 | **S01-5** | Home / a selected earlier day | That day's entries, totals and water; the goal in force on it; the meals heading and water sheet name the day | A strip tile, previous week, or Home after a commit on that day | Tile / Today / week controls; a meal's add action → O01 bound to that day; an entry → S07-3 | runtime + story |
| 53 | **S01-6** | Home / streak | The count of consecutive logged days with its explanation | Any Home | The control → the rule sheet | runtime + story |
| 54 | **S02-11** | Search / Recipes · catalogue | The complete catalogue with its count while the query is empty, narrowed by the criteria snapshot | Search recipes / View all / Browse all from S03; the Recipes tab | Filters → O02; a card → S08-2 (Search origin); Clear all filters | runtime + story |
| 55 | **S06-4** | Manual entry / correction draft | Step 1 prefilled from a barcode or photo record with its provenance stated | Edit label values (S07-4) / Edit nutrition values (S07-5) | Continue to portion → S06-5; Back → the review; Cancel → origin (asks only when edited) | runtime + story |
| 32′ | **S06-5** | Manual entry / Step 2 · portion and meal | Identity summary + Edit, amount with steps and presets, live result, meal, day, Add to {meal} | Continue to portion with a valid draft | Add to {meal} → one entry, Home on the bound day; Edit / Back → S06-1 kept; Cancel → O08 | runtime + story |
| 42′ | **O08** | Discard changes? (overlay) | The shared exit confirmation | Cancel, Back out of the task, or browser Back on a changed draft | Keep editing (focused) / Escape / backdrop → unchanged; Discard changes → the recorded origin | runtime + story |

### 12.7 Verification record (2026-09-05, R1–R6 close-out)

| Command | Result |
| --- | --- |
| `npm run tokens:check` | 268 tokens validated; generated output current |
| `npm run icons:check` | 33 symbols from 27 glyphs; generated output current |
| `npm run typecheck` | clean |
| `npm run test:unit` | 18 files, 110 tests passed (day keys, streak, goal history, portion draft, discovery, photo store, persistence v2 migration, matching, daily log) |
| `npm run build` | ok |
| `node scripts/verify/runtime-walkthrough.mjs` | 86 passed, 0 failed; no console or page errors; 64 captures |
| `npm run build-storybook` | ok |
| `node scripts/verify/storybook-captures.mjs` | 158 captures, 0 with problems (55 states, 23 variants, 80 component/foundation) |
| `npx vitest run --project storybook` (isolated) | 82 files, 504 tests passed; a11y addon at `error` |
...
Fixed during verification: a literal backspace byte inside a story regex (`/^g\b/` written through a heredoc) that could never match; the O04 story asserting visibility on a visually hidden radio input; the App two-step story matching the recommended Lentil soup recipe card as well as the logged entry; the photo Cancel-during-analysis story racing its 60 ms fixture under a loaded parallel run (now a 400 ms fixture for that story only); four capture rows still naming the retired 2 × 2 MethodSheet stories; the S01-5 story ending on Today instead of the earlier day it documents.
...
- **D-58 Day strip.** (Refined in §13.9: unselected and future days are text on the canvas without a tile; only the selected day is filled.) One scrolling radio row of whole weeks from a week before the earlier of (today − 3 weeks) and the selected day to the end of today's week; selecting the earliest tile reveals another week, so all history stays reachable by touch; arrow keys, Home and End move the selection; mouse drag scrolls (a drag never counts as a tap); the selected tile is scrolled into view; the row bleeds to the shell's edges and owns its overflow.
- **D-59 Daily nutrition.** (Refined in §13.9: the calorie card uses the macro cards' surface — canvas, decorative border, card radius — and leads by typography.) `DailyNutrition` = calorie card + three `NutritionMacros` cards, one 8 px step apart, no wrapper border; the calorie card's only action is `Set targets` / `Edit targets`; the bar precedes the legend (`● 790 consumed · 40 %` / `Target 2,000 kcal`); without a target the card shows the logged amount alone and never a percentage, remainder or "on track"; a macro card carries a bar only with a user target and never looks disabled without one.
...
### 13.9 Refinements (2026-09-05, after the H2 close-out)

Scope: three focused refinements on the same branch; nothing else changed.

- **D-68 Day strip tiles.** An unselected day is weekday and number on the canvas — transparent background and border, no shadow — with the 48 px target, the control radius (focus ring, selected fill), the hover surface and the scrolling unchanged; a future day is muted disabled text (`state.disabled-text`) with no tile; the selected day keeps its filled action-blue tile and today keeps its dot. Existing tokens only.
- **D-69 Targets apply from today until changed.** Verified against the existing effective-dated history rather than changed: `setGoalFrom(history, todayKey, goal)` records a period from the local day of saving whatever day Home shows; the same day's repeated saves replace that day's period; a removal is a `null` period from today; earlier periods, entries and water stay; the history persists in record version 2 and follows the local midnight through `useLocalDayKey`. The editor's copy now says "Applies from today until you change it." on the entry sheet, the editor and the estimate review. No per-day override, schedule or formula change.
- **D-70 Calorie card surface.** The calorie card takes the macro cards' surface (canvas, decorative border, card radius) instead of the grey grouped surface; prominence comes from the 40/48 figure. Four separate containers, aligned edges, one 8 px gap, no wrapper, shadow or tint. `Surface` props only; no other consumer changed.

Verification (this refinement): `npm run typecheck` clean; `npm run test:unit` 20 files, 133 tests (the goal-history tests already cover carry-forward, same-day replacement, a cleared period and the v1 migration); `npm run build`; `node scripts/verify/runtime-walkthrough.mjs` 115 passed, 0 failed, no console or page errors — seven checks added: targets survive a reload with their provenance and the new copy, a past day shows no target of its own, saving while viewing a past day changes nothing for that day, the new target replaces today's earlier save from today, removing targets keeps the entries and water, the stored history keeps every period (estimate → replacement → removal) rather than rewriting the past, and a target saved before a mocked local midnight stays in force on the new day; `npm run build-storybook` and `node scripts/verify/storybook-captures.mjs` 176 captures, 0 with problems; the affected story files (day strip, daily nutrition, Home, Lane A, targets sheet, App) 65 tests passed, including new assertions that unselected and future days have no background or border while the selected day is filled, and that the calorie card and a macro card share background and border colours; detector `[]`; contrast matrix 73 pairs, 0 failing. Rendered inspection: Home first use, active target, exceeded target (150 over, protein 90 / 80 g), the earlier day, 320 and 320 at 200 %, the day strip alone at 320 / 200 %. Not exercised: real device, screen reader, an overnight session (midnight is mocked).

## 14. Target flow redesign (2026-09-06) — the route sheet, three focused steps, one review / editor, effective dates

### 14.1 Baseline and scope

Baseline: `feat/redesign-home-recipes-targets` at `89d88e5` (H2 close-out and the §13.9 refinements). Scope: the target setup, estimation, review and editing flow only. Kept: the estimate domain (`energy-estimate.ts`, `macro-presets.ts`), the goal history model, Home's calorie card as the one targets action, the recommendation, every other screen. Retired: `TargetsSheet` (the sheet-to-sheet wizard), its stories and its seven capture rows.

### 14.2 Anatomy and token brief (frozen before implementation)

- **Entry**: one `ModalSheet` — `Set daily goal` / "Choose how to set your target." / two `MethodOption` rows (Calculator tile, "Help me estimate — Use your body stats and activity."; quiet PencilSimple, "I know my goal — Enter your daily calories.") / one compact note on `feedback.info.surface` with `feedback.info.foreground` text and the circled Info glyph ("You can change your targets anytime from Home.") / Close; no Cancel footer.
- **Focused shell**: `FocusedFlowLayout` for every later screen. New `FocusedBar` pattern: a `48px | 1fr | 48px` grid, min-height 56 px, top safe area owned once; Back (IconButton) at the start, the trailing Help (Question) at the end, the step count centred (`Text` numeric, "Step 1 of 3" visually hidden). The 28/36 heading sits 24 px below the bar; one primary large block button in the footer with a text `Cancel setup` / `Cancel changes` beneath (the food footers' arrangement).
- **Spacing**: `spacing.label-to-control` (8) inside a field, `space.16` between related fields, `space.24` between sections, `space.12` between selection cards and between the two tiles; `space.16` card padding; `space.4` title-to-helper.
- **SelectionCard** (component): label around a visually hidden native radio; `radius.card`; border `border.decorative` → `action.primary` when selected with `action.selected-surface`; reserved 24 px indicator slot (Check icon when checked); focus ring through `:has(.input:focus-visible)`; `presentation="tile"` centres a short label in an equal-width pair.
- **UnitField** (feature): the label at the start of a wrapping label row, a compact `SegmentedControl` (`flex: 0 0 auto; white-space: nowrap`) at its end, the `Input` — or the equal ft / in pair on a two-column grid — beneath; the error or helper for the whole group.
- **Review**: `Text` `label` secondary "Estimated" / "Adjusted from the estimate" → the 40/48 `main-result` figure → `body` secondary "kcal/day" → a small secondary `Adjust` → `supporting` "An estimate you can adjust." — a column centred on the container. Adjust replaces the figure with an `AmountField` (max 18 rem) and `Done` / `Use the estimate`.
- **Macro rows**: no grouped surface; `border.decorative` hairlines between rows; the 8 px nutrient marker (`nutrition.*.accent`) before the name; grams in `metric-inline`, the share in `supporting` secondary.
- **Effective date**: "Starts **Today**" with a text `Change`; a `TextField type="date" min={today}`; `Use today`; one `supporting` sentence from `describeStart`. Pending change: a `supporting` line with `Cancel scheduled change`.
- **Remove targets**: `Button variant="destructive"` (the error surface / foreground tokens) → `ConfirmDialog` "Remove daily targets".
- **HelpDialog** (pattern): native `<dialog>`, `radius.sheet`, heading + body + `<details>` "Calculation details" + Close; Escape and Close return focus.

### 14.3 Decisions (continuing §13.4)

- **D-71 One sheet, then full screen.** Only the route choice is a sheet. Every later screen is a focused step on `FocusedFlowLayout` with no logo / date / streak header and no bottom navigation, the mobile container kept on desktop. Reason: the wizard-in-a-sheet stacked three form steps and a review inside an overlay that could not own focus, the keyboard or the safe areas cleanly; the app already had the focused shell every food task uses.
- **D-72 The task owns one draft.** `TargetsDraft` (route, About you inputs, activity, goal, the reviewed estimate, calories, adjusted flag, macro choice, start date) lives in App as `targetsTask`; the steps are `targets-*` entries on the existing flow stack, so Back, Help, Edit details, Recalculate, browser Back and the exit guard reuse the food tasks' machinery. Dirtiness is a snapshot comparison against the task's start; a discard confirmation appears only when dirty.
- **D-73 Help carries the method; the path does not.** Formula names, years, equations and the "nothing is saved" repetition moved into `HelpDialog` (short body) and its *Calculation details* disclosure. Eligibility limits stay in the path: "Adults 19 and over." under Age (and the refusal under 19), the 1,200 kcal floor as a warning under the goal cards the moment Lose weight is chosen, with `Set it myself` into the manual editor keeping every value.
- **D-74 Adjust in place, labelled.** The review shows one number. Adjust turns that number into the field (no duplicate figure); a changed value relabels the block "Adjusted from the estimate", `Use the estimate` restores it, and the saved goal carries `adjusted: true`. Preset grams follow the adjusted calories; Custom grams stay as entered.
- **D-75 Maintenance is never a gain target.** Gain shows the maintenance estimate with "This is the maintenance estimate. Add the surplus you want above it." — no calculated surplus, no rate or timeline anywhere.
- **D-76 Effective date as an extension of "from today".** `Starts Today` by default; `Change` allows today or later. Rules: one record per start date; at most one pending change (a future save replaces any other scheduled period — `cancelPending` then `setGoalFrom`; an immediate save keeps it — `setGoalFrom(…, { keepLater: true })`); `Cancel scheduled change` is a confirmed change applied at once; `Remove targets` clears today and the scheduled change after one confirmation naming both; the date is revalidated at Save. The copy table lives in `docs/ux/targets-and-estimation.md`.
- **D-77 Save writes before it applies.** `saveRecord` now reports success; the app writes the new history to the device first and only then sets state, so a refused write keeps the whole draft with "Not saved" and `Try again`; a second activation during a save is ignored. Home confirms with one toast and returns to the originating day; from an earlier day the toast says "Targets saved from today. Nothing changes yesterday."
- **D-78 The entry decision is today's.** `Set targets` on an earlier day opens today's editor when today has targets (the save replaces today's period), the route sheet only when none exist — the label follows the shown day, the action follows the record.
- **D-79 Equal tiles, whole-card selection.** Female / Male as two `SelectionCard` tiles (`flex: 1 1 0%`, centred, the indicator slot reserved); activity and goal as full-width cards with one-line descriptions shortened without changing meaning; no auto-advance.
- **D-80 History mirroring counts traversals, not entries.** `useHistoryStack` assumed one popstate per entry when a multi-step drop called `history.go(-n)`; one traversal fires one event, so the leftover ignores swallowed the next real Back presses (surfaced by the five-step save from the review). Fixed to count one per traversal, for both the drop and the guard's restore.

### 14.4 Impeccable use (recorded before each invocation)

| Problem | Tool / scope | Finding |
| --- | --- | --- |
| Confirm the craft floor and the operate posture before editing the flow | `reference/operate.md`, `reference/craft-floor.md` (context loader once per session) | Operate: refine within the accepted system; no bolder / colorize / delight / overdrive / onboard. Applied as the token brief above. |
| Deterministic anti-pattern scan of the new sources | `scripts/detect.mjs --json` on `src/features/calorie-calculator/targets`, `SelectionCard`, `FocusedBar`, `HelpDialog`, `MacroTargetsEditor.*`, `CalorieBudgetBar.tsx` | `[]` (no findings). |
| The edit hook on every written UI file | design hook (auto) | No deterministic design-quality issues on any of the written files. |

### 14.5 Rendered quality gate

Batched review of the runtime captures (02, 50b–50i) and the Storybook rows (39–39s, O09, O09-2) at 390 / 393, 320 at 200 %, 430, with the safe-area fixture, and the reduced-motion story. Measured: the step count centred on the container (walkthrough and FocusedBar story assert < 1.5 px), the two tiles within 1 px of each other, the 40/48 figure centred within 2 px, the segmented control at the end of its label row. One consolidated fix batch: the unit control was allowed to shrink and wrapped its labels ("ft + / in") → `flex: 0 0 auto; white-space: nowrap`; the steps' `Cancel setup` sat in the content while the editor's sat in the footer → moved to the footer beneath the primary action. Confirmation round: re-rendered 39b, 39c, 39d, 39o and the runtime rows; both fixes hold at 320 / 200 % (the label row wraps, the control drops under its label at full size). Known capture artefact: full-page images place the sticky footer mid-image (manifest capture rule).

### 14.6 Verification record (2026-09-06)

- `npm run tokens:check` — 268 tokens validated, generated output current. `npm run icons:check` — 34 symbols from 28 glyphs, current (Question added; Info and Check already present).
- `npm run typecheck` — clean. `npm run test:unit` — 20 files, 135 tests (two new goal-history cases: the pending change and its replacement; keepLater, removal and `cancelPending`).
- `npm run build` — clean. `node scripts/verify/runtime-walkthrough.mjs` — **135 passed, 0 failed, no console or page errors** (20 checks added for §14; two existing checks made date-robust: the day-strip check on a Sunday, the history check on the local day key).
- `npm run build-storybook` + `node scripts/verify/storybook-captures.mjs` — **188 captures, 0 with problems** (19 new rows 39–39s; the two O09 state rows re-rendered).
- `npm run test:storybook` — **88 files, 547 tests passed** (final run after every fix; the walkthrough was rerun on the same build: 135 passed, 0 failed). The targets, SelectionCard, HelpDialog, FocusedBar, Lane A and Home story files needed three rounds: spies on the harness args, `alertdialog` for the confirmations, the story-only dirtiness override, `fireEvent.change` for the date input, and the ref-based focus after a refused save.
- `node scripts/verify/contrast-matrix.mjs --check` — 73 pairs, 0 failing.
- Not exercised: a real device, a screen reader, an overnight session (midnight is mocked), the browser's own unload prompt (documented limit), desktop wide-browser rendering beyond the 430 capture.

### 14.7 Checkpoint

Implementation, stories, docs (DESIGN.md §Targets flow, `docs/ux/targets-and-estimation.md`, README revision T1, PRODUCT.md, ui-contract, low-fi inventory note, the two skills), the manifest and the curated evidence are complete on this branch; every script above was run on the committed tree. Nothing remains open from the brief; no `main` change, merge or history rewrite was made.
