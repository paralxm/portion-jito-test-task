...
Implemented 2026-09-05 (revision H2, docs/design/hifi-decisions.md §13): setting targets is an optional task with two routes — I know my goal (calorie target, nutrition preference, Save) and Help me estimate (About you → Lifestyle → Goal → Estimated daily target → Save targets); Back keeps every step's draft, dismissing with edits asks through the shared Discard changes? confirmation, Cancel leaves the saved targets untouched, and Save applies once from today. Recipe discovery starts from a featured recipe or a collection and completes on Recipe Details as before; View all and Browse all continue into Search's Recipes scope with the preferences shown as chips, and Back restores discovery with its state and scroll.
