...
Implemented 2026-09-06 (target flow redesign, docs/design/hifi-decisions.md §14): the targets sheet is reduced to the route choice; both routes continue as focused full-screen steps (About you 1/3, Your activity 2/3, Your goal 3/3 with a compact step bar and contextual Help; the review "Your daily target" and the editor as one screen with Adjust in place), targets start today or on a chosen later date with at most one scheduled change, and removal clears today's targets and the scheduled change after one confirmation — see docs/ux/targets-and-estimation.md.
