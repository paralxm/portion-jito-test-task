...
Implemented 2026-09-06 (the target flow, docs/design/hifi-decisions.md §14): O09 keeps its identity as the route-choice sheet (the only sheet of the flow, Close and no Cancel footer); O09-2 is the full-screen review "Your daily target" on the focused layout; the three estimate steps, the manual editor, the edit editor, the Help dialog and the removal confirmation are focused states of the same task rather than sheet steps. The inventory count is unchanged (57).
