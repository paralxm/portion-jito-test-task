...
| `NavigationBar` | The app's three fixed root destinations in one compact group, plus the separate circular Log food action | `aria-current="page"` on the active destination; inactive destinations are icon-only with `aria-label`; Log food is a plain labelled button | Home / Search / Recipes + Log food |
