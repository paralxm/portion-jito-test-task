...
- Only the route choice is a sheet: `TargetsEntrySheet` (feature) is a `ModalSheet` titled `Set daily goal` — "Choose how to set your target." — with two `MethodOption` rows, **Help me estimate** (prominent, "Use your body stats and activity.") and **I know my goal** (quiet, "Enter your daily calories."), and one compact informational note beneath ("You can change your targets anytime from Home.", a circled Info glyph on the information surface — reassurance, never a warning). Close at the top right; no Cancel footer.
- Everything after the choice is a focused full-screen step on `FocusedFlowLayout`: no logo, date or streak header, no bottom navigation, the mobile container kept on desktop, the footer owning the bottom safe area with one primary action (`Continue`, `Review estimate`, `Save targets`) and, where leaving is allowed, a quiet text `Cancel setup` / `Cancel changes` beneath it.
- `FocusedBar` (pattern) is the steps' header: a 48 px Back slot at the start, a 48 px Help slot at the end, and the step count (`1/3`, `2/3`, `3/3`, tabular; "Step 1 of 3" for assistive technology) centred on the container between the two equal slots. The screen heading (28/36) sits below the bar; the review and the editor show no step count. Focus lands on the heading; no field opens the keyboard on its own.
- `HelpDialog` (pattern) is a native `<dialog>` with a heading, one to three sentences and a *Calculation details* `<details>` disclosure; Close and Escape return focus to Help. Formula names, years, equations and provenance banners live only inside the disclosure.
- **About you** (1/3): Age (`AmountField`, years, "Adults 19 and over."); the sex the equation uses as two `SelectionCard`s in `presentation="tile"` — equal widths, 12 px gap, centred labels, a reserved indicator slot; `UnitField` for Height (`cm | ft + in`) and Weight (`kg | lb`): the label at the start of the label row, a compact `SegmentedControl` at its end, the input — or the equal ft / in pair — full width beneath. Errors sit beside their field; Continue focuses the first. Spacing: 8 px label-to-control, 16 px between related fields, 24 px between sections.
- **Your activity** (2/3, "Think about a typical week.") and **Your goal** (3/3): four (three) `SelectionCard`s in a 12 px stack — 16 px padding, a 4 px title-to-helper gap, a 24 px indicator slot at the end so nothing shifts, the selected surface and boundary plus the check mark, the whole card clickable, no auto-advance. An unsupported loss goal is an `InlineMessage` (warning tone) under the cards with `Set it myself`.
- **Your daily target** (review) and the editor are one `TargetEditorScreen` (feature) in three modes. Review: a small `Estimated` label, the 40/48 figure centred with `kcal/day` beneath, a small secondary `Adjust` that turns the same number into a field in place (`Adjusted from the estimate` once it differs; `Use the estimate` returns), "An estimate you can adjust.", the `Lose weight · Lightly active` summary with `Edit details`. Manual and edit: `Daily calories` as a field; edit adds the compact source row (`Estimated · Lose weight · Lightly active` + `Recalculate`, or `Entered by you` + `Estimate instead`) and `Remove targets` in the destructive treatment. All modes share `MacroTargetsEditor` — the preference chips wrapping cleanly, then Protein / Carbs / Fat as hairline-separated rows with the nutrient marker, the grams and the share as secondary text (no grouped surface, no AMDR panel) — a `Separator`, and `EffectiveDateControl` (`Starts Today · Change`, a native date input from today on, `Use today`, one truthful sentence; the scheduled change stated once with `Cancel scheduled change`).
- `Remove targets` confirms with `ConfirmDialog` (`Remove daily targets` — "Food, water and past targets will stay." plus the scheduled change it cancels — `Keep targets` / `Remove targets`); a dirty exit opens the shared `DiscardChangesDialog` with the targets wording. A refused write keeps the draft with an error-tone `InlineMessage` above `Try again`.
- Home states the one scheduled change on the calorie card (`Scheduled: 1,750 kcal from tomorrow.`) beneath the target line while today is shown.
