...
- `DayStrip` (feature, after H-REF 1) is one compact row of 48 px day tiles — weekday over date — exposed as a radio group: today marked with a dot and named `today`, the selected tile filled in action blue with on-action text, days after today disabled and named `not available yet`. The row itself is the navigation: it scrolls by touch, trackpad, mouse drag and keyboard (arrow keys, Home, End), bleeds to the shell's edges, and keeps the selected tile in view; there is no calendar container and no previous / next week button. A slim caption row above carries the selected month and, while another day is selected, the `Today` text action. Tiles never shrink below 3 rem or 48 px.
...
- `DailyNutrition` (feature, after H-REF 1) is one coordinated section: the dominant calorie card (light surface, grouped radius) and, one 8 px step beneath it, three macro cards (canvas, decorative border, card radius) sharing its outer width. No wrapper border is drawn around the group, and nothing else on Home carries a targets action.
