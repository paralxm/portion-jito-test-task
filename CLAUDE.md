...
- Initial launch is the empty **Calculate** workspace. There is no separate Home dashboard. Keep one bottom row: **Calculate | Search | Recipes | + Add food**. These are three destinations and one trailing button; plus is never selected and never duplicated as a floating action. Use current-page semantics for navigation; selected-tab semantics only for an actual tab pattern.
- S01 = Calculate; S02 = shared Search with Food/Recipes scopes; S03 = query-free recipe browsing; S04 = Barcode; S05 = Photo; S06 = Manual entry; S07 = Food review; S08 = Recipe details. These are design IDs, not implemented URL routes.
