...
- Targets: optional, set or edited through the targets sheet (docs/ux/targets-and-estimation.md) — a manual path with suggested macro presets, or an estimate path built on the 2023 Dietary Reference Intakes energy equations with an explicit review before saving. No mandatory first-use step, no automatic average target.
